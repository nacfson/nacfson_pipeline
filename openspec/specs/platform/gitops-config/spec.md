...
### Requirement: Local-first Git bootstrap

Ansible SHALL verify whether Forgejo is installed before Flux bootstrap. When
Forgejo is absent, Ansible SHALL install the pinned bootstrap workload, create
and seed the local deployment repository from committed project history, and
register Flux's read-only deploy key. When Forgejo is already installed,
Ansible SHALL skip workload installation without replacing persistent data.
Flux SHALL use the Forgejo repository from its first reconciliation; an
external Git host MUST NOT be required.

#### Scenario: Clean node installs Forgejo before Flux

- **WHEN** bootstrap runs on a ready k3s node without the Forgejo Deployment
- **THEN** Ansible installs Forgejo, waits for readiness, seeds `main`, verifies
  the repository over SSH, and only then installs Flux against Forgejo

#### Scenario: Existing Forgejo installation is preserved

- **WHEN** bootstrap runs and the Forgejo Deployment already exists
- **THEN** Ansible reports the existing installation, skips workload
  installation, preserves its persistent data, verifies repository readiness,
  and continues with Flux

#### Scenario: External Git is unavailable

- **WHEN** GitHub, GitLab, and other external Git hosts are unreachable
- **THEN** Forgejo bootstrap and Flux reconciliation still succeed using the
